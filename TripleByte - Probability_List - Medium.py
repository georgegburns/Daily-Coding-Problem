...
def probabilityList(nums : list, probs : list):
    TEMP = []
    for i, prob in enumerate(probs):
        TEMP.append(prob + sum(probs[:i]))
    CHANCE = rand.randrange(0,10)/10
    for i, chance in enumerate(TEMP):
        if chance >= CHANCE:
            RESULT = nums[i]
            break
    return RESULT
...
print(probabilityList(TEST,PROBS))